...
function Row({title, movies}: props) {
    ...
    <div className='h-40 space-y-0.5 md: space-y-2'>
        ...
        <div className='group relative md:-ml-2'>
            ...
            hover: scale-125 group-hover:opacity-100 ${!isMoved && 'hidden'}`} onClick={() => handleClick("left")} 
            />

        <div ref={rowRef} className='flex items-center space-x-0.5 overflow-scroll scrollbar-hide md: space-x-2.5 md:p-2 '>
            {movies.map((movie)=>(
            ...
            hover: scale-125 group-hover:opacity-100' onClick={() => handleClick ("right")}
            />
        </div>
    </div>
  )
